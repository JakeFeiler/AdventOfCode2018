...
def run_carts(carts, paths):
    ...
    while True:
        ...
        #Check to see if there is 1 cart left
        if len(carts) == 1:
            #Need to reverse coordinates
            coords = (carts[0].coords[1], carts[0].coords[0])
            return coords
        ...
        for moving_cart in carts:
            ...
            #Collision happened, find and remove the offending carts
            if overlap == 2:
                carts = [cart for cart in carts if cart.coords != moving_cart.coords]
